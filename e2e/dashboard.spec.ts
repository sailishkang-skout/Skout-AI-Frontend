...
test.describe("Dashboard", () => {
  test("shows workspace stats and quick actions", async ({ page }) => {
    ...
    await expect(page.getByTestId("quick-action-search")).toBeVisible({ timeout: 45_000 });
    await expect(page.getByTestId("quick-action-lists")).toBeVisible({ timeout: 45_000 });
  });
